...
for (i in papernames) {
  ...
  #Get median
  median_switch <- median(paper$avg_switchrate)
  paper$cat_switch = ifelse(paper$avg_switchrate>= median_switch,1,0)
  ...
  write_rds(paper, paste("data/PreprocessedPaperData/cpt_", tolower(papername), ".rds.bz2", sep =""), compress = "bz2")
}
