...
dat <- data %>% 
  ...
  rename(
    ...
    sample = trial, # sample number 
    attended = option # sampled option
    
  ) %>% 
  
  mutate( # issue: some problems treat the same outcome as different outcome or did not specify the second probability 
  ...
  mutate(
    ...
    n_switch = sum(switch, na.rm = TRUE) ,
    r_switch = round(n_switch/((n_sample - 1)), 2) ,
...
# issue: for some trials, the sampled probabilities do not add up to 1 as the same outcome as different outcome or did not specify the second probability 
# exclude paper for now but fix issue later
